...
const ALL_GRADES = [...PASSING_GRADES, 'F', 'DN', 'W', 'IC', 'IP', 'NP', 'WP', 'WF', 'NF'];
//
...
export function parseRecordHTML(html, debug = false) {
  ...
  $('tr').each((rowIdx, row) => {
    ...
    for (let i = 0; i < cellTexts.length; i++) {
      ...
      if (val >= 1 && val <= 6 && cellTexts[i].trim().length <= 2) {
        credits = val;
        break;
      }
    }
    ...
    // if name is junk, clear name and grade (route will fix name from plan)
  ...
  // dedupe - keep version with real name, prefer graded version only if name is real

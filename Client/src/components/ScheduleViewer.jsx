  ...
  return (
    <div className="schedule-viewer">
      ...
      <div className="schedule-nav">
        ...
        <div className="schedule-tabs">
          {schedules.map((s, i) => (
            ...
            >
              جدول {i + 1}
              <span className="tab-credits">{s.totalCredits} ساعة</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="schedule-content">
        ...
        {/* additional courses without sections */}
        {current.additionalCourses && current.additionalCourses.length > 0 && (
          <div className="additional-courses">
            <h4>مواد مطلوبة بدون شعب في الجدول ({current.additionalCourses.length})</h4>
            <div className="additional-list">
              {current.additionalCourses.map((c, i) => (
                <div key={i} className="additional-course-item">
                  <span className="course-code">{c.code}</span>
                  <span className="course-name">{c.name}</span>
                  <span className="course-credits">{c.credits} ساعات</span>
                </div>
              ))}
            </div>
            <div className="total-credits-all">
              إجمالي الساعات مع جميع المواد: <strong>{current.totalCreditsWithAll}</strong>
            </div>
          </div>
        )}
      </div>

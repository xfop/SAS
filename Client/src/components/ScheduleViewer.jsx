    ...
    return (
      <div className="schedule-viewer">
        <div className="no-schedules">
          <h3>⚠️ لا يوجد جدول ممكن هذا الفصل</h3>
          {summary?.coursesWithSections === 0 ? (
            <p>لا توجد مواد مؤهلة متاحة في جدول هذا الفصل. تحقق من أن ملف الجدول صحيح أو تواصل مع مرشدك الأكاديمي.</p>
          ) : (
            <p>
              يوجد <strong>{summary?.coursesWithSections}</strong> مادة متاحة في الجدول لكن جميع شعبها تتعارض مع بعضها — لا توجد تركيبة ممكنة بدون تعارض.
              تواصل مع مرشدك الأكاديمي.
            </p>
          )}
          {summary?.requiredCourses?.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <strong>المواد المطلوبة:</strong>
              <ul style={{ marginTop: 6, paddingRight: 20 }}>
                {summary.requiredCourses.map(c => (
                  <li key={c.code} style={{ color: c.hasSection ? '#2563eb' : '#6b7280' }}>
                    {c.code} — {c.name} ({c.credits} ساعة){c.hasSection ? ' ✓ متاحة في الجدول' : ' — لا توجد شعب'}
                  </li>
                ))}
              </ul>
            </div>

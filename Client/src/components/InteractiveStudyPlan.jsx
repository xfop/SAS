...
import { COURSE_LEVELS, LEVEL_NAMES } from '../constants';
import { getPlan } from '../utils/api';

function InteractiveStudyPlan() {
  ...
  // FIXME: might be slow for large plans but works for now
  function getAllPrerequisites(code, visited) {
    ...
    for (var i = 0; i < course.prerequisites.length; i++) {
      ...
      var more = getAllPrerequisites(course.prerequisites[i], visited);
  ...
  for (var i = 1; i <= 10; i++) {
    coursesByLevel[i] = [];
    if (COURSE_LEVELS[i]) {
      for (var j = 0; j < COURSE_LEVELS[i].length; j++) {
        var code = COURSE_LEVELS[i][j];
  ...
  return (
    <div className="interactive-study-plan">
      <button className="plan-toggle-btn" onClick={() => setShowPlan(!showPlan)}>
       علوم الحاسب - الخطة الدراسية (2014)
        <span className="toggle-arrow">{showPlan ? '▲' : '▼'}</span>
      </button>
